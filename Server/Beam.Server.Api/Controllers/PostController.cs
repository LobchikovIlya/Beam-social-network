...
public class PostController : ControllerBase
{
    ...
    public async Task<ActionResult<List<PostDto>>> GetAllAsync()
    {
        ...
        if (userIdFromToken == null) return BadRequest("User ID is missing from the token.");

        if (!Guid.TryParse(userIdFromToken, out var userId)) return BadRequest("Invalid user ID format.");
        ...
        foreach (var post in posts) post.UserName = users.FirstOrDefault(u => u.Id == post.UserId)?.Tag ?? "Unknown";
