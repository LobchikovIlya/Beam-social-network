...
public class PostLikesController : ControllerBase
{
    ...
    public PostLikesController(IPostLikeService postLikeService,
        IPostLikeNotificationService postLikeNotificationService, IPostService postService)
    {
    ...
    public async Task<IActionResult> ToggleLike(Guid postId)
    {
        ...
        if (userId == Guid.Empty) return Unauthorized(); // Если нет пользователя, возвращаем ошибку авторизации
    ...
    public async Task<IActionResult> GetLikes([FromQuery] PostLikeFilter filter)
    {
        ...
        if (userIdFromToken == null) throw new BadRequestException("User ID is missing from the token.");
